...
export async function add(components: string[]) {
  ...
  for (const name of components) {
    ...
    try {
      ...
      if (!item) {
        spinner.fail(
          chalk.red(`Component "${name}" not found in Polare UI registry.`)
        )
        console.log(
          chalk.dim(
            `  Run ${chalk.cyan("npx polareui list")} to see available components.\n`
          )
        )
        continue
      }
      ...
      for (const file of item.files) {
        ...
        if (exists) {
          spinner.warn(
            chalk.yellow(`${file.target} already exists — skipping.`)
          )
          continue
        }
      ...
      spinner.succeed(
        chalk.green(`${item.title} added to components/ui/${name}.tsx`)
      )
      ...
      if (item.dependencies?.length) {
        ...
        console.log(
          chalk.dim(
            `  Run: ${chalk.cyan(`pnpm add ${item.dependencies.join(" ")}`)}\n`
          )
        )
      }
    } catch (err) {
